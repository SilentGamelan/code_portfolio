//  Use the reduce Method to Analyze Data.js
...
it's possible to show that both filter and map can be derived as a special application of reduce
let's practice using reduce first
---
...
// I tried to find a way of only using .reduce()
// Struggle without using a global variable, which defeats the object - trying to be as functional as possible.
// Originally used .filter().map() chain, then realised wasn't necessary.
//
// it IS! possible to use a single .map() but perhaps not very elegant code?
...
var baverageRating = watchList
                            .filter( item => item.Director === "Christopher Nolan")
                            .reduce((a, c, i, arr) => {
                                a = a + Number(c.imdbRating);
                                return i < arr.length - 1 ? a : (a / arr.length);
                            }, 0);
